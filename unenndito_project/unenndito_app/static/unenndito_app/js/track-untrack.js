    ...
    for (var i=0; i<check_box_track.length; i++) {
        ...
        check_box_track[i].onclick = function(e) {
            if (e.target.checked == true) {
                ...
                xhttp.open("GET", `${origin_}/trackbillspreferences/?bill_id=` + e.target.id.substring(6) + "&option=" + track_label[e.target.index].dataset.onLabel, true);
                xhttp.send();
                ...
                xhttp.open("GET", `${origin_}/trackbillspreferences/?bill_id=` + e.target.id.substring(6) + "&option=" + track_label[e.target.index].dataset.offLabel, true);
                xhttp.send();
